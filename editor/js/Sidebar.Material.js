...
function SidebarMaterial(editor) {
	...
	*/
	...
	materialClearcoatRoughnessRow.add(
		new UIText(strings.getKey("sidebar/material/clearcoatroughness")).setWidth("90px")
	);
	...
	materialClearcoatNormalMapRow.add(
		new UIText(strings.getKey("sidebar/material/clearcoatnormalmap")).setWidth("90px")
	);
		...
		if (material) {
			...
			*/
			...
			if (
				...
				Math.abs(material.clearcoatRoughness - materialClearcoatRoughness.getValue()) >= epsilon
			) {
				editor.execute(
					new SetMaterialValueCommand(
						editor,
			...
			if (material.map !== undefined) {
				...
				if (objectHasUvs) {
					...
					if (material.map !== map) {
						editor.execute(
							new SetMaterialMapCommand(editor, currentObject, "map", map, currentMaterialSlot)
						);
					}
				} else {
			...
			if (material.matcap !== undefined) {
				...
				if (objectHasUvs) {
					...
					if (material.matcap !== matcap) {
						editor.execute(
							new SetMaterialMapCommand(editor, currentObject, "matcap", matcap, currentMaterialSlot)
						);
					}
				} else {
					if (mapEnabled) textureWarning = true;
			...
			if (material.alphaMap !== undefined) {
				...
				if (objectHasUvs) {
					...
					if (material.alphaMap !== alphaMap) {
						editor.execute(
							new SetMaterialMapCommand(editor, currentObject, "alphaMap", alphaMap, currentMaterialSlot)
						);
					}
				} else {
					if (mapEnabled) textureWarning = true;
			...
			if (material.bumpMap !== undefined) {
				...
				if (objectHasUvs) {
					...
					if (material.bumpMap !== bumpMap) {
						editor.execute(
							new SetMaterialMapCommand(editor, currentObject, "bumpMap", bumpMap, currentMaterialSlot)
						);
					}
			...
			if (material.normalMap !== undefined) {
				...
				if (objectHasUvs) {
					...
					if (
						...
						var value = [materialNormalScaleX.getValue(), materialNormalScaleY.getValue()];
						editor.execute(
			...
			if (material.clearcoatNormalMap !== undefined) {
				...
				if (objectHasUvs) {
					...
					if (
						material.clearcoatNormalScale.x !== materialClearcoatNormalScaleX.getValue() ||
			...
			if (material.envMap !== undefined) {
				...
				if (material.envMap !== envMap) {
					editor.execute(
						new SetMaterialMapCommand(editor, currentObject, "envMap", envMap, currentMaterialSlot)
					);
				}
			}
			...
			if (material.lightMap !== undefined) {
				...
				if (objectHasUvs) {
					...
					if (material.lightMap !== lightMap) {
						editor.execute(
							new SetMaterialMapCommand(editor, currentObject, "lightMap", lightMap, currentMaterialSlot)
						);
					}
				} else {
			...
			if (material.aoMap !== undefined) {
				...
				if (objectHasUvs) {
					...
					if (material.aoMap !== aoMap) {
						editor.execute(
							new SetMaterialMapCommand(editor, currentObject, "aoMap", aoMap, currentMaterialSlot)
						);
					}
			...
			if (material.side !== undefined) {
				...
				if (material.side !== side) {
					editor.execute(
						new SetMaterialValueCommand(editor, currentObject, "side", side, currentMaterialSlot)
					);
				}
			}
			...
			if (material.size !== undefined) {
				...
				if (material.size !== size) {
					editor.execute(
						new SetMaterialValueCommand(editor, currentObject, "size", size, currentMaterialSlot)
					);
				}
			}
			...
			if (material.blending !== undefined) {
				...
				if (material.blending !== blending) {
					editor.execute(
						new SetMaterialValueCommand(editor, currentObject, "blending", blending, currentMaterialSlot)
					);
				}
			}
