...
function Viewport(
	...
	if (config.labelRenderer) {
		...
		controls = new PointerLockControls(camera, container.dom);
		// controls.lock();
		initPointerLock(container.dom);
		editor.scene.add(controls.getObject());
		controlsEvent();
	}
	...
	function animate() {
		...
		// }
		// prevTime = time;
		if (controlsEnabled) contrclsUpdate();
